...
object Node extends NodeInfo {
  ...
  def disk(stats: JsValue): JsObject = {
    ...
    val usedPercent = if (totalInBytes > 0) 100 - (100 * (freeInBytes.toFloat / totalInBytes.toFloat)).toInt else 0
    Json.obj(
  ...
  def loadAverage(nodeStats: JsValue): JsNumber = {
    val load = (nodeStats \ "os" \ "cpu" \ "load_average" \ "1m").asOpt[Float].getOrElse(0f)
    JsNumber(BigDecimal(load.toDouble))
  }

  def cpuPercent(nodeStats: JsValue): JsNumber = {
    val cpu = (nodeStats \ "os" \ "cpu" \ "percent").asOpt[Int].getOrElse(0)
